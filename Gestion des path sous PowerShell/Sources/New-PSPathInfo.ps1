      ...
      $O=New-Object PSObject -Property @{
         ...
         #Bien que dans ce cas IsAbsolute=$false, on ne peut en conclure que le nom de chemin est relatif.
              ...
              #ATTENTION le nom du drive peut contenir des jokers ( 'C*' ) ou des espaces, exemple : ' C:\Temp\foo.txt'
               ...
               #cf. ResolvedPSPath
               Win32PathName=$null #todo renommer TargetPath ( peut être un fichier, une ruche, un objet AD,...) ou spécialiser l'objet
             }          
...
#<UNDEF %DEBUG%>         
      ...
      #todo n'échapper que ceux qui le sont déjà ?
      if ($isLiteral)
...
#<UNDEF %DEBUG%>          
      ...
      $Infos| 
        Add-Member -Membertype Scriptmethod -Name GetFileName { #todo rename GetPathName peut être un conteneur ou un item
...
#en
# $PSPathInfo=New-MyPSPathInfo -LiteralPath $Object
Function New-MyPSPathInfo {
  [CmdletBinding(DefaultParameterSetName='Path')]
  param(
